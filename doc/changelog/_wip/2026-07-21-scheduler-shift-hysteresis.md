...
- 修复 MasterySync 对进行中精专反复生成立即 REFRESH_TIME：数据库不再隐藏已过期状态，所有时间统一为本地无时区语义，并以单一未来任务或 30 分钟退避收敛。
- 精专完成、换助理和 ETA 变化按 plan_key 去重；异常路径仍保留下一检查点，避免队首紧循环。
- 修复排班页加载无条件清空 dorm_order；后端同时校验当前 Free 床位集合，旧标签页提交空值时保留现有有效 11 床顺序，拓扑变化时安全重建。
- 修复 try_add_release_dorm 错用残留循环变量判断高效干员，避免宿舍释放结果依赖字典末项。
- 立即贸易站跑单在首次加速前检查无人机下限和 OCR 错误值；未知活动道具不再中断整次仓库扫描。
- Windows 正式依赖、隔离 MOWER_DATA_DIR 的相关套件 67/67 通过，最后修订定向 9/9 通过；WebUI 生产构建完成 6679 个模块。
- 本批只发布代码与 WebUI，不修改 plan.json、conf.yml 或 SQLite；线上存在工作位低于 9.6，因此不与副表恢复同批执行。
